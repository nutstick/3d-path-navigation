...
QuantizedMeshGeometry::QuantizedMeshGeometry(QuantizedMeshTile * t, const Map3D & map, const MapToPixel & mapToPixel, QNode * parent)
	: QGeometry(parent)
{
	...
	double xMinWgs = t->extent.left();
	double yMinWgs = t->extent.bottom();
	double widthWgs = t->extent.width();
	double heightWgs = t->extent.height();
	QPoint ptMinProjected = QPoint(map.originX, map.originY);
	...
	for (int i = 0; i < vertexCount; ++i)
	{
		...
		// QPoint ptProjected = terrainToMap.transform(xWgs, yWgs);
		QPoint ptProjected = QPoint(xWgs, yWgs);
		QPoint ptFinal(ptProjected.x() - ptMinProjected.x(), ptProjected.y() - ptMinProjected.y());
		...
		*vbptr++ = -ptFinal.y();
		...
		QPoint uv = mapToPixel.transform(ptProjected) / map.tileTextureSize;
		// texture coords
		*vbptr++ = uv.x();
		*vbptr++ = uv.y();
	}
...
QuantizedMeshTile * QuantizedMeshGeometry::readTile(int tx, int ty, int tz, const QRect & extent)
{
